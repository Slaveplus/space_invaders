...
### PlayerState 추출
- `GameStateManager` 내부에 직접 존재하던 공격력/속도/HP/스킬포인트 수치를 `PlayerState` 클래스로 분리.
...
## 2. 앞으로의 추천 단계
1. Game 루프 내에서 `GameNetworkAdapter.tick()` 호출 지점 추가 (authoritative 스냅샷 생성 or 수신 적용).
2. 입력 처리: `InputManager`가 직접 ship 제어 대신 `PlayerInput` DTO 생성 -> 어댑터 전송 -> 서버 authoritative 반영 구조 도입.
3. 서버/클라이언트 분기:
...
6. 발사/스킬 사용: 현재 Game이 직접 수행 -> Command(Event) 객체화 후 서버 검증.
7. 룸 로비(`room` 패키지)와 gameplay 연결: 룸 진입 시 `localPlayerId`를 GameStateManager에 설정, 네트워크 어댑터를 방/서버 연결 정보로 교체.

## 3. 코드 전환 가이드
| 기존 | 변경 권장 | 비고 |
|------|-----------|------|
| GameStateManager.attackPower 등 필드 | PlayerState 통해 접근 | 레거시 메서드 제거 예정 |
| Game.tryToFire() 직접 발사 | 서버 Command 전송 -> 서버에서 생성 | 클라이언트는 예측 샷 선택적 |
...
## 6. 추후 삭제 예정 (정리 대상)
- GameStateManager의 단일 플레이어 전용 getter/setter.
- Game 내부에서 상태 숫자 직접 조작하는 코드 (PlayerState 경유하도록 수정 예정).
