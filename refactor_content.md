...
**냄새에 대한 자세한 내용:**
`createRemoteEntity` 메서드는 수많은 `case`를 가진 거대한 `switch` 문으로 구성되어 새로운 엔티티 타입 추가 시 수정이 매우 불편했습니다(OCP 위반). 또한 `update`, `render`와 같은 메서드들도 여러 책임(상태 업데이트, 렌더링, 오버레이 그리기 등)을 한 곳에서 처리하여 인지 복잡도가 높았습니다.
...
private void initializeEntityFactories() {
    ...
    entityFactories.put("Alien", (s, m) -> new RemoteAlienEntity(s, m));
    entityFactories.put("Shot", (s, m) -> new RemoteShotEntity(s, m));
    // ... 다른 엔티티 팩토리 등록
}
...
**냄새:** **긴 메서드 (Long Method)**
**냄새에 대한 자세한 내용:**
`RoomListCanvas`와 `RoomLobbyCanvas`의 `render()` 메서드는 배경, 제목, 버튼, 플레이어 목록, 채팅 등 화면의 모든 UI 요소를 그리는 코드를 포함하고 있어 매우 길고 복잡했습니다. 이로 인해 특정 UI 요소의 렌더링 코드를 찾거나 수정하기가 어려웠습니다.

**대상:**
*   `RoomListCanvas.java`의 `render()` 메서드
*   `RoomLobbyCanvas.java`의 `render()` 메서드

**적용 기법:**
**메서드 추출 (Extract Method):** `render()` 메서드의 로직을 UI 요소의 종류에 따라 `drawBackground()`, `drawTitle()`, `drawRoomList()`, `drawPlayerList()`, `drawChat()` 등과 같은 여러 개의 작은 private 메서드로 분리했습니다. `render()` 메서드는 이들을 순서대로 호출하는 역할만 맡게 되어, 전체 렌더링 흐름을 쉽게 파악할 수 있게 되었습니다.
